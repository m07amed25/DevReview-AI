    ...
    <main className="dark min-h-screen bg-zinc-950 text-zinc-50 overflow-x-hidden">
      ...
      <section className="relative overflow-hidden pt-28 pb-20 sm:pt-36 sm:pb-28">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-sm backdrop-blur-md mb-8">
          ...
          <motion.p initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.22 }} className="mx-auto mt-6 max-w-2xl text-lg sm:text-xl text-zinc-400 leading-relaxed font-light" style={{ textWrap: "balance" }}>
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.34 }} className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Button asChild size="lg" className="h-12 px-8 rounded-full bg-white text-zinc-900 hover:bg-zinc-100 font-semibold">
              <Link href="/sign-up">Get started free <ArrowRight className="ml-2 h-4 w-4" /></Link>
            </Button>
            <Button asChild variant="ghost" size="lg" className="h-12 px-8 rounded-full text-zinc-300 hover:text-white hover:bg-white/5 border border-white/10">
      ...
      <section className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-24 sm:py-32">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <RevealSection>
            ...
            <p className="mt-5 text-zinc-400 text-lg leading-relaxed">We believe code review is the highest-leverage moment in software development — yet it is still largely manual, inconsistent, and painfully slow.</p>
            <p className="mt-4 text-zinc-400 text-lg leading-relaxed">Our mission is to give every developer an always-available AI reviewer that catches bugs, surfaces security risks, and provides actionable feedback — before a single human reviewer is pulled away from building.</p>
            ...
            <div className="relative rounded-2xl border border-white/10 bg-white/3 p-8 overflow-hidden">
              ...
              <div className="relative space-y-5">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div><p className="text-sm font-semibold text-white">Code Catch</p><p className="text-xs text-zinc-500">just now · auth/login.ts</p></div>
                ...
                <p className="text-sm text-zinc-400 leading-relaxed">⚠️ Unsanitized query parameter passed directly to authentication logic. This is vulnerable to injection attacks (OWASP A03:2021). Validate and sanitize before use.</p>
                <div className="flex items-center gap-3 pt-2 border-t border-white/5">
                  <Badge variant="outline" className="text-xs border-white/10 text-zinc-500">CWE-20</Badge>
                  <Badge variant="outline" className="text-xs border-white/10 text-zinc-500">OWASP A03</Badge>
      ...
      <section className="border-t border-white/5 bg-zinc-900/30">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-24 sm:py-32">
          <RevealSection className="text-center mb-16">
            ...
            <h2 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-white">How we got here</h2>
            <p className="mt-4 text-zinc-400 max-w-xl mx-auto text-lg">From a late-night frustration to a product used by hundreds of teams.</p>
          </RevealSection>
                ...
                <RevealSection key={milestone.year} delay={i * 0.1}>
                  <div className="flex gap-6 sm:gap-8">
                    <div className="relative shrink-0 flex flex-col items-center">
                      <div className="h-12 w-12 rounded-xl border border-white/10 bg-zinc-800 flex items-center justify-center shadow-lg z-10">
                        <milestone.icon className={`h-5 w-5 ${milestone.color}`} />
                      </div>
                      {i < milestones.length - 1 && <div className="flex-1 w-px bg-white/5 mt-3 sm:hidden" />}
                    </div>
                    <div className="pb-2">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {values.map((value, i) => (
            <RevealSection key={value.title} delay={i * 0.07}>
              <div className={`group relative h-full rounded-2xl border border-white/5 bg-white/2 p-6 transition-all duration-300 hover:bg-white/4 hover:border-white/10 hover:shadow-xl ${value.shadow} ${value.border}`}>
      ...
      <section className="border-t border-white/5 relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-24 sm:py-32 text-center">
          <RevealSection>
            ...
            <p className="text-zinc-400 text-lg max-w-xl mx-auto mb-10 leading-relaxed">Connect your GitHub repository and get your first AI-powered review in under two minutes. No credit card required.</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Button asChild size="lg" className="h-14 px-10 text-base rounded-full bg-white text-zinc-900 hover:bg-zinc-100 font-bold shadow-[0_0_40px_rgba(255,255,255,0.1)]">
                <Link href="/sign-up"><Github className="mr-2 h-5 w-5" />Connect GitHub — it&apos;s free</Link>
              </Button>
              <Button asChild variant="ghost" size="lg" className="h-14 px-10 text-base rounded-full text-zinc-300 hover:text-white hover:bg-white/5 border border-white/10">
