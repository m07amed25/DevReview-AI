...
function generateSequenceDiagram(fileContents: Record<string, string>): {
  definition: string;
  nodes: DiagramNode[];
  edges: DiagramEdge[];
  warning?: string;
  ...
  for (const [filePath, content] of Object.entries(fileContents)) {
    ...
    // Detect exported async functions / methods as participants
    ...
    // Detect call expressions: callee(args)  →  caller → callee
  ...
  // Deduplicate participants; fall back to a generic pair if none found
  ...
  for (const p of allParticipants) {
    ...
    nodes.push({
      ...
      label: p,
      type: "ACTOR",
      detail: { description: p, interactions: [] },
    });
  }

  // Deduplicate and limit interactions
  ...
  // If no interactions were captured, emit a minimal placeholder
  if (edgeIdx === 0) {
