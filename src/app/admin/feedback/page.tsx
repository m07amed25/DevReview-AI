...
import { ThumbsUp, ThumbsDown, ChevronLeft, ChevronRight, MessageSquare, ExternalLink } from "lucide-react";
...
export default function AdminFeedbackPage() {
  ...
  const handleRatingChange = (val: string) => {
    setRating(Number(val) as RatingFilter);
    setPage(1);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold tracking-tight">PR Review Feedback</h1>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Showing {data?.feedbacks.length ?? 0} of {data?.total ?? 0} total entries
          </CardDescription>
        </CardHeader>
          ...
          {isLoading ? (
            ...
            <div className="divide-y">
              {data?.feedbacks.map((fb) => (
                <div key={fb.id} className="flex flex-col gap-4 p-6 transition-colors hover:bg-muted/50">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="font-semibold text-sm">{fb.user.name ?? "Anonymous User"}</span>
                    ...
                    <div className="flex items-center gap-2">
                      <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${
                        ...
                        <span>{fb.rating === 1 ? "Helpful" : "Not Helpful"}</span>
                      </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between pt-2">
                      <div className="flex flex-col gap-1">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Attached Review</span>
                        <div className="flex items-center gap-2">
                          <Link 
                          ...
                          <span className="text-[10px] text-muted-foreground">{fb.review.repository.fullName}</span>
                        </div>
                      </div>
                    </div>
                  </div>
